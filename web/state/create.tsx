...
export function createStore<TTree extends NestedTree>(
  ...
  const store: Store<State, Action> = createRedux<State, Action, {}, {}>(
    reduce,
    composeWithDevTools ? composeWithDevTools({ name })(applyMiddleware(saga.saga)) : applyMiddleware(saga.saga)
  )

  function withState<TFromState, TProps = {}>(
    map: StateMap<State, Action, TFromState>,
    comp: Comp<TFromState & TProps & { dispatch: Dispatcher<Action>; children?: any }>
  ): React.FC<TProps> {
  ...
  function withDispatch<TProps = {}>(comp: Comp<TProps & { dispatch: Dispatcher<Action> }>): React.FC<TProps> {
...
type Comp<T> = React.ComponentType<T> | React.Component<T> | React.FunctionComponent<T> | React.FC<T>
